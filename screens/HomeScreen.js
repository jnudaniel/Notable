...
export default class HomeScreen extends React.Component {
 ...
 writeNote = (props) => {
  ...
  switch (possTags.indexOf(tag)) {
    case 0: return <Text style={styles.definitionText}> {content}</Text>;
    case 1: return <Text style={styles.sectionText}>-{content}-</Text>;
    case 2: return <Text style={styles.importantText}> {content}</Text>;
    case 3: return <Text style={styles.examText}> {content}</Text>;
...
PrintNotes() {
  const notes = "#def This is an example of a notes page.\n#section We start with a section"
  const lines = notes.split("\n");
 
   const listItems = lines.map((line) =>
     <Text line = {line} style = {styles.definitionText}> {line} </Text>
   )
return (
    <Text>{listItems}</Text>
  )
